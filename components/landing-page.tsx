...
function ChartTooltip({ active, payload }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-ink/10 bg-[#f8f4ec]/92 px-4 py-3 text-left shadow-card backdrop-blur-[2px]">
      <p className="text-[0.68rem] uppercase tracking-[0.2em] text-ink/45">
        {point.label}
      </p>
...
function FeatureGlyph({
  ...
  glyph: (typeof metricCards)[number]["glyph"];
}) {
  if (glyph === "memory") {
    return (
      <svg
  ...
  return (
    <div className="flex items-end gap-3">
      {[0.28, 0.62, 0.42, 0.86].map((height, index) => (
        <div
          key={height}
          className="relative h-44 w-4 overflow-hidden rounded-full bg-[#3f3f3f]"
        >
          <div
...
export function LandingPage() {
  ...
  const [marketSource, setMarketSource] = useState<"cache" | "fresh" | null>(
    null,
  );
  ...
  const loadMarket = useCallback(async (forceFresh = false) => {
    try {
      ...
      const res = await fetch(
        `/api/market-intelligence${forceFresh ? "?fresh=1" : ""}`,
        {
      ...
      setMarketError(null);
      setMarketSource(
        res.headers.get("x-market-source") === "fresh" ? "fresh" : "cache",
      );
    } catch (err) {
      setMarketError(
  ...
  const avgSavings =
    avgPublic && avgArbiter ? Math.max(avgPublic - avgArbiter, 0) : 0;

  const chartData = useMemo<ChartPoint[]>(() => {
    const points = market?.staysDataPoints?.length
      ? market.staysDataPoints
      : (market?.flightDataPoints ?? []);
  ...
  const signals = useMemo(() => {
    ...
    const sorted = [...primaryStays].sort(
      (a, b) => a.publicRate - b.publicRate,
    );
    ...
    return [
      ...
      marketSource === "fresh"
        ? "Source: live market fetch"
        : "Source: cached market fetch",
    ];
  ...
  return (
    <main
      id="top"
      className="relative overflow-hidden bg-canvas text-ink min-h-screen"
    >
      ...
      <section
        id="hero"
        className="relative z-20 isolate overflow-hidden pt-28 lg:pt-40"
      >
            ...
            >
              <div>
                <h1 className="hero-shadow mt-2 text-[2.8rem] leading-[0.98] tracking-[-0.04em] text-ink sm:text-[3.9rem] lg:text-[4.7rem]">
                  Galileo: live enterprise travel sourcing without the manual
                  chase.
                </h1>
                <p className="hero-shadow mt-6 max-w-2xl text-base leading-8 text-ink/66 sm:text-lg font-medium">
                  Corporate hotels, flights, and event blocks sourced
                  continuously from one agentic workflow, with live market
                  pricing wired directly into the decision layer.
                </p>
              </div>
              ...
              <div className="mt-16 sm:mt-32 mb-20 lg:mb-24">
                <motion.a
                  ...
                  whileHover={{
                    scale: 1.05,
                    boxShadow: "0 20px 40px -15px rgba(47,47,47,0.15)",
                  }}
                  whileTap={{ scale: 0.95 }}
            ...
            <div className="relative lg:pl-8 lg:pt-3">
              ...
              >
                <p className="text-xl font-bold leading-tight text-ink">
                  Increase in hotel bookings when using automated follow-up
                  methods.
                </p>
                <p className="mt-3 max-w-xl text-sm leading-7 text-ink/66 font-medium">
                  A company leveraging HubSpot's speed-to-lead automation saw a
                  25% boost in qualified leads through immediate follow-up
                  emails.
                </p>
                ...
                <div className="mt-5 rounded-[1.75rem] border-2 border-ink/20 bg-[#f8f4ec]/62 p-5 backdrop-blur-[1px]">
                  <div className="relative h-[17.5rem] overflow-hidden">
                    ...
                    <div className="absolute left-[4.35rem] top-[0.9rem] text-[0.74rem] font-bold text-ink">
                      25%
                    </div>
                    <div className="absolute left-[4.35rem] top-[6.2rem] text-[0.68rem] font-bold text-ink/78">
                      12%
                    </div>
                    <div className="absolute left-[4.75rem] bottom-[3.2rem] text-[0.68rem] font-bold text-ink/78">
                      0%
                    </div>
      ...
      {isRevealed && (
        ...
        >
          <section
            ...
            className="scroll-mt-20 mx-auto max-w-7xl px-6 pb-20 pt-16 mt-8 lg:px-10 lg:pb-24 lg:pt-24 lg:mt-12"
          >
            <motion.div
              initial="hidden"
              animate="visible"
            ...
            >
                ...
                <div className="mt-5 space-y-5">
                  {topVendors.length ? (
                    topVendors.map((vendor) => (
                      <div
                        key={vendor.label}
                        className="border-l border-ink/18 pl-4"
                      >
                        <p className="text-lg leading-7 text-ink">
                          {vendor.label}
                        </p>
                        <p className="mt-1 text-sm leading-6 text-ink/58">
                          Market ${vendor.publicRate} · Galileo $
                          {vendor.arbiterRate}
                        </p>
                      </div>
                    ))
                    ...
                    <p className="text-sm text-ink/52">
                      {marketError
                        ...
                        : "Loading live hotel quotes."}
                    </p>
                  )}
                </div>
                <p className="mt-7 text-xs uppercase tracking-[0.16em] text-ink/46">
                  {market?.fetchedAt
                    ? `Updated ${formatTimestamp(market.fetchedAt)}`
              ...
              <div>
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <p className="text-[0.72rem] uppercase tracking-[0.22em] text-ink/58">
                      Live Market Graph
                    </p>
                    <p className="mt-2 text-xl text-ink">
                      Chicago travel pricing
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="mt-6 h-64 lg:h-72">
                  {chartData.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart
                        data={chartData}
                        margin={{ top: 14, right: 10, left: 10, bottom: 24 }}
                      >
                        <defs>
                          <linearGradient
                            id="publicRateFill"
                ...
                <div className="mt-5 space-y-2 text-xs text-ink/56">
                  {signals.length ? (
                    ...
                    <div>
                      {marketError
                        ? "Live sync failed. Check the API token."
          ...
          <section
            id="capabilities"
            className="mx-auto max-w-7xl px-6 pb-24 pt-10 lg:px-10 lg:pt-16"
          >
            <div className="flex flex-col gap-6">
              ...
              >
                <div className="space-y-6">
                  <p className="text-sm uppercase tracking-[0.24em] text-ink/62">
                    Performance Metrics
                  </p>
                  ...
                  <p className="max-w-3xl text-base leading-8 text-ink/62">
                    According to engine.com, the leading travel management
                    website.
                  </p>
                </div>
              </motion.article>
              ...
              <div className="grid gap-6 lg:grid-cols-3">
                {metricCards.map((card, index) => (
                  ...
                  >
                    ...
                    <h3 className="mt-5 text-[1.9rem] leading-tight tracking-[-0.03em] text-ink">
                      {card.title}
                    </h3>
          ...
          >
            ...
            >
              <div className="max-w-3xl">
                <p className="text-sm uppercase tracking-[0.26em] text-ink/42">
                  Scale
                </p>
                <h2 className="mt-4 text-[2.4rem] leading-[1.03] tracking-[-0.04em] text-ink sm:text-[3.6rem]">
                  Built for enterprise travel programs that cannot afford manual
                  lag.
                </h2>
              </div>
              <p className="max-w-md pt-2 text-base leading-8 text-ink/52">
                Hotels, flights, and event travel stay inside one negotiation
                memory so the system scales without losing context.
              </p>
            </motion.div>
